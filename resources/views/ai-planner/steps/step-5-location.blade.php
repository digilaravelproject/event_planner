...
<div x-show="currentStep === {{ $stepNumbers['service_area'] ?? -1 }}" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-8">
    <div class="space-y-2">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#850625]/10 text-[#850625] text-xs font-extrabold uppercase tracking-widest">
            ...
            <span>Step {{ str_pad((string) ($stepNumbers['service_area'] ?? 0), 2, '0', STR_PAD_LEFT) }} / {{ count($plannerSteps) }} • Select Location(s)</span>
        </div>
        ...
        <template x-for="(loc, idx) in optionsFor('service_area', []).map((val, i) => {
            ...
            <div @click="
                ...
                :class="planner.locations.includes(loc.id) 
                    ? 'border-[#850625] ring-2 ring-[#850625]/20 shadow-xl scale-[1.01]'
                ...
                <div class="absolute inset-0 w-full h-full pointer-events-none opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                    :class="planner.locations.includes(loc.id) ? '!opacity-100' : ''">
                ...
                <div class="relative z-10 flex items-center justify-between">
                    <div class="w-8 h-8 rounded-xl flex items-center justify-center text-sm transition-all"
                        :class="planner.locations.includes(loc.id) ? 'bg-white/85 backdrop-blur-md text-[#850625] border border-white' : 'bg-rose-50 text-[#850625]'">
                ...
                <div class="relative z-10 space-y-1">
                    <h4 class="font-bold text-slate-950 text-sm sm:text-base leading-snug" x-text="loc.title"></h4>
