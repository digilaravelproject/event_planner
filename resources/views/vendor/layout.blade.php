...
<body class="h-screen w-screen overflow-hidden text-slate-600 antialiased bg-[#f8f9fe] flex relative">
    ...
    <aside id="vendor-sidebar" class="fixed inset-y-0 left-4 my-4 flex w-64 flex-col rounded-2xl bg-white border border-slate-100 shadow-xl" style="z-index: 60;">
        ...
        <nav class="flex-1 space-y-1.5 px-4.5 py-6 overflow-y-auto">
            ...
            <!-- Request Quote -->
            <a href="{{ route('vendor.quote-requests.index') }}" class="flex items-center gap-x-3.5 rounded-xl px-4 py-3 text-xs font-bold tracking-tight transition-all duration-150 {{ str_starts_with($currentRoute, 'vendor.quote-requests') ? 'bg-orange-50 text-orange-600 shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800' }}">
                <span class="flex h-7 w-7 items-center justify-center rounded-lg shadow-sm {{ str_starts_with($currentRoute, 'vendor.quote-requests') ? 'bg-orange-500 text-white' : 'bg-orange-50 text-orange-500' }}">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
    ...
    <!-- Main Content Canvas -->
    <div id="main-content-area" class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden relative lg:pl-[18.5rem]">
