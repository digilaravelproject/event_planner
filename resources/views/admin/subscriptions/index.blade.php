...
<div class="space-y-6 -mt-16 relative z-30">
    ...
    <div>
        ...
        <p class="text-sm text-white/80 mt-1 font-medium">Configure pricing packages, billing intervals, and service capability limits for the SaaS portal.</p>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-1 space-y-6">
            @if(request()->filled('edit'))
                ...
                @if($editingPlan)
                    <!-- Edit Plan -->
                    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100 space-y-5">
                        <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                            <h3 class="text-lg font-bold text-slate-800">Edit Plan</h3>
                            <a href="{{ route('admin.subscriptions.index') }}" class="text-xs font-semibold text-rose-600 hover:text-rose-500">Cancel Edit</a>
                        </div>
                        ...
                        <form action="{{ route('admin.subscriptions.update', $editingPlan) }}" method="POST" class="space-y-4">
                            ...
                            <div>
                                ...
                                <select name="interval" id="interval" required
                                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-600 focus:outline-none focus:ring-1 focus:ring-rose-500 focus:border-rose-500 transition-all">
                            ...
                            <div class="space-y-2.5">
                                ...
                                @for($i = 0; $i < 5; $i++)
                                    ...
                                    <input type="text" name="features[]" value="{{ old('features.'.$i, $fVal) }}" placeholder="Feature item {{ $i + 1 }}"
                                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-rose-500 focus:border-rose-500 transition-all">
                                @endfor
                            </div>

                            <button type="submit" class="w-full py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-sm font-semibold transition shadow-sm hover:shadow active:scale-[0.99]">
        ...
        <div class="lg:col-span-2 space-y-6">
            <h3 class="text-lg font-bold text-white px-2">Pricing Cards Catalog Preview</h3>

            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                @forelse($subscriptions as $plan)
                    <!-- Plan Card -->
                    <div class="rounded-2xl bg-white border border-slate-100 p-6 shadow-sm flex flex-col justify-between relative transition duration-200 hover:-translate-y-0.5 hover:shadow-md">
                        <!-- Actions float menu -->
                        <div class="absolute top-4 right-4 flex items-center gap-2">
                            <a href="{{ route('admin.subscriptions.index', ['edit' => $plan->id]) }}" class="text-indigo-400 hover:text-indigo-600 transition" title="Edit plan">
                            ...
                            <form action="{{ route('admin.subscriptions.destroy', $plan) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this subscription plan?')" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-rose-400 hover:text-rose-600 transition focus:outline-none" title="Delete plan">
                                    <svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <!-- Card Header -->
                        <div class="mb-5">
                            <span class="inline-flex items-center rounded-md bg-rose-50 px-2 py-1 text-xs font-bold text-rose-700 ring-1 ring-inset ring-rose-600/10 uppercase tracking-wide">
                                {{ $plan->name }}
                            </span>
                ...
                @empty
                    <div class="col-span-2 rounded-2xl bg-white p-12 text-center text-slate-400 font-medium shadow-sm ring-1 ring-slate-100">
