...
<div class="mt-6 space-y-6">
    ...
    <section class="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex items-start justify-between gap-4"><div><div class="text-[10px] font-extrabold uppercase tracking-[.18em] text-amber-600">Customer health</div><h2 class="mt-1 text-lg font-black text-slate-900">Feedback overview</h2></div><span class="rounded-xl bg-amber-50 px-3 py-2 text-sm font-black text-amber-700">{{ number_format($metrics['averageRating'], 1) }}/5</span></div>
            <div class="mt-5 rounded-2xl bg-slate-50 p-5"><div class="text-3xl font-black text-slate-900">{{ number_format($metrics['pendingFeedback']) }}</div><div class="mt-1 text-xs font-semibold text-slate-500">feedback items awaiting review</div><a href="{{ route('admin.feedback.index', ['status' => 'pending']) }}" class="mt-4 inline-flex text-xs font-extrabold text-[#3950a2]">Review feedback →</a></div>
            <div class="mt-6"><div class="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Newest users</div><div class="mt-3 space-y-3">@forelse($recentUsers as $recentUser)<div class="flex items-center gap-3"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-50 text-xs font-black text-[#3950a2]">{{ str($recentUser->name)->substr(0, 1)->upper() }}</span><div class="min-w-0 flex-1"><div class="truncate text-xs font-extrabold text-slate-700">{{ $recentUser->name }}</div><div class="truncate text-[10px] text-slate-400">{{ $recentUser->subscription?->name ?? 'No active plan' }}</div></div><span class="text-[10px] text-slate-400">{{ $recentUser->created_at->diffForHumans(null, null, true) }}</span></div>@empty<div class="text-xs text-slate-400">No users yet.</div>@endforelse</div></div>
        </article>
    </section>
</div>
@endsection
