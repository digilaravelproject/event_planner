...
<div class="max-w-xl mx-auto space-y-6 -mt-16 relative z-30">
    <!-- Header -->
    <div>
        <h1 class="text-3xl font-extrabold text-white tracking-tight font-serif-display">Manage AI Credentials</h1>
        <p class="text-sm text-white/80 mt-1 font-medium">Configure OpenAI API keys and parameters to integrate AI capabilities into the frontend planner.</p>
        ...
        <form action="{{ route('admin.ai.manage.save') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="password" name="openai_api_key" id="openai_api_key" value="{{ old('openai_api_key', $apiKey) }}" placeholder="sk-proj-..."
                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-rose-500 focus:border-rose-500 transition-all font-mono">
                <p class="text-[10px] text-slate-400 mt-1.5 font-medium">Your API key is stored securely in the database settings matrix.</p>
                @error('openai_api_key') <p class="text-xs text-rose-500 mt-1.5 font-medium">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                @error('openai_model') <p class="text-xs text-rose-500 mt-1.5 font-medium">{{ $message }}</p> @enderror
            </div>
