...
<div class="admin-page space-y-5">
    ...
    <div class="grid gap-4 sm:grid-cols-3"><div class="admin-card p-5"><p class="text-[10px] font-bold uppercase text-slate-400">Active inventory rate</p><p class="mt-2 text-2xl font-black text-emerald-600">{{ $health['active_rate'] }}%</p><div class="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full bg-emerald-500" style="width:{{ min(100,$health['active_rate']) }}%"></div></div></div><div class="admin-card p-5"><p class="text-[10px] font-bold uppercase text-slate-400">Plans generated</p><p class="mt-2 text-2xl font-black text-indigo-600">{{ number_format($health['plans_created']) }}</p><p class="mt-3 text-xs text-slate-400">Original user plans in period</p></div><div class="admin-card p-5"><p class="text-[10px] font-bold uppercase text-slate-400">Planned value</p><p class="mt-2 text-2xl font-black text-[#3950a2]">₹{{ number_format($health['planned_value']/100000,2) }}L</p><p class="mt-3 text-xs text-slate-400">Estimated plan totals in period</p></div></div>

    @php($chartTitles=['categories'=>'Current Vendors by Category','registrations'=>'Vendor Registrations — Last 6 Months','selected_categories'=>'Planning Demand by Category'])
