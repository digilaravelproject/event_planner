...
<div class="max-w-4xl mx-auto space-y-6 mt-16 relative z-30">
    <!-- Breadcrumb Header -->
    <div class="flex items-center gap-2 text-sm text-slate-100 font-semibold">
        <a href="{{ route('admin.users.index') }}" class="hover:text-blue-200 transition">Users</a>
    ...
    <div>
        <h1 class="text-3xl font-extrabold text-white tracking-tight">Edit User Profile</h1>
        <p class="text-sm text-white/80 mt-1">Update profile information and subscription plans for {{ $user->name }}.</p>
    ...
    <div class="rounded-2xl bg-white p-6 sm:p-8 shadow-sm ring-1 ring-slate-100">
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-6">
            ...
            <div class="border-b border-slate-100 pb-3">
                <h3 class="text-sm font-bold text-blue-600 uppercase tracking-wider">1. Account Details</h3>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="mobile_number" id="mobile_number" value="{{ old('mobile_number', $user->mobile_number) }}" required
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all @error('mobile_number') border-rose-500 bg-rose-50/10 @enderror">
            ...
            <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                ...
                <button type="submit" class="rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold px-4.5 py-2.5 transition shadow-sm shadow-blue-900/10 active:scale-[0.99]">
                    Save Changes
                </button>
            </div>
        </form>
