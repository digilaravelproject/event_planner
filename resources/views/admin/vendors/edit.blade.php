...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-2 text-sm text-slate-500 font-semibold">
        ...
        <span class="text-slate-800">Edit Vendor Profile</span>
    </div>

    <div>
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Vendor Profile</h1>
        <p class="text-sm text-slate-500 mt-1">Update profile information, geographic locations, and venue parameters for {{ $vendor->business_name }}.</p>
        ...
        <form action="{{ route('admin.vendors.update', $vendor) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" placeholder="Brief explanation of services, portfolios, and themes..."
                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all @error('description') border-rose-500 bg-rose-50/10 @enderror">{{ old('description', $vendor->description) }}</textarea>
