...
<div class="space-y-6 mt-6 relative z-30">
    <!-- Header -->
    <div>
        <h1 class="text-3xl font-extrabold text-slate-800 tracking-tight">System Masters</h1>
        <p class="text-sm text-slate-500 mt-1 font-semibold">Configure algorithms, moderate listings, adjust subscriptions, and review analytics.</p>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <form action="{{ route('admin.master-registries.store') }}" method="POST" class="space-y-3">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" required placeholder="e.g. Payment Statuses" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-[#3950a2]/20 focus:border-[#3950a2] transition-all placeholder-slate-400 text-slate-700 font-medium">
                        </div>
                        <button type="submit" class="w-full py-2 rounded-xl bg-[#3950a2] hover:bg-[#2c3e80] text-white text-xs font-bold transition mb-4 cursor-pointer shadow-sm active:scale-[0.99]">
                            + Create Registry
                        </button>
                    </form>
                </div>
                <div class="space-y-1">
                    @foreach($allowedTypes as $key => $title)
                        ...
                        <a href="{{ route('admin.system-masters.index', ['type' => $key]) }}" 
                            class="flex items-center justify-between rounded-xl px-4 py-3 text-sm font-semibold transition-all duration-200 {{ $isActive ? 'bg-[#3950a2] text-white shadow-md shadow-[#3950a2]/15' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                            <span>{{ $title }}</span>
                            <span class="text-xs font-bold px-2.5 py-0.5 rounded-full {{ $isActive ? 'bg-[#2c3e80] text-white' : 'bg-slate-100 text-slate-500' }}">
                                {{ $count }}
                            </span>
                        </a>
                    @endforeach
        ...
        <div class="lg:col-span-2">
            <div class="rounded-xl bg-white p-6 shadow-sm border border-slate-200/60 space-y-6">
                ...
                <div class="flex items-center justify-between pb-4 border-b border-slate-100">
                    <div class="flex-1 mr-4">
                        ...
                        <div id="registry-title-display" class="flex items-center gap-3">
                            <div>
                                <h2 class="text-xl font-extrabold text-slate-800 tracking-tight">Managing: {{ $allowedTypes[$activeType] ?? 'No Registry Selected' }}</h2>
                            ...
                            <div class="flex items-center gap-1.5 ml-2">
                                <button type="button" class="text-slate-400 hover:text-[#3950a2] transition p-1 cursor-pointer" onclick="showEditRegistryForm()" title="Rename Registry">
                                ...
                                <form action="{{ route('admin.master-registries.destroy', $activeRegistryObj->id) }}" method="POST" class="inline" onsubmit="return confirm('WARNING: Deleting this Master Registry will permanently delete all its parameters and items. Are you sure you want to proceed?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-slate-400 hover:text-red-600 transition p-1 cursor-pointer" title="Delete entire registry">
                                        <svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        ...
                        <form id="registry-title-edit-form" action="{{ route('admin.master-registries.update', $activeRegistryObj->id) }}" method="POST" class="hidden flex items-center gap-2 mt-1">
                            ...
                            <input type="text" name="title" value="{{ $allowedTypes[$activeType] ?? '' }}" required
                                class="bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#3950a2]/20 focus:border-[#3950a2] transition-all font-semibold text-slate-800">
                            
                            <button type="submit" class="rounded-xl bg-[#3950a2] hover:bg-[#2c3e80] px-4 py-2 text-xs font-bold text-white shadow-sm transition cursor-pointer">
                                Save
                            </button>
                            <button type="button" class="rounded-xl bg-slate-100 hover:bg-slate-200 px-4 py-2 text-xs font-bold text-slate-600 transition cursor-pointer" onclick="hideEditRegistryForm()">
                                Cancel
                            </button>
                        </form>
                        @endif
                    ...
                    <span class="inline-flex items-center rounded-md bg-emerald-50 px-2.5 py-1 text-xs font-bold text-[#00c689] border border-emerald-100 uppercase tracking-wider shrink-0">
                ...
                <form action="{{ route('admin.system-masters.store') }}" method="POST" class="flex gap-3">
                    ...
                    <div class="flex-1">
                        <input type="text" name="label" required placeholder="Add new registry to {{ $allowedTypes[$activeType] }}" 
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#3950a2]/20 focus:border-[#3950a2] transition-all">
                        @error('label')
                            <p class="text-xs text-red-500 mt-1 font-medium">{{ $message }}</p>
                        @enderror
                    </div>

                    <button type="submit" class="inline-flex items-center gap-1.5 rounded-xl bg-[#3950a2] px-5 py-3 text-sm font-semibold text-white transition-all duration-200 hover:bg-[#2c3e80] shadow-sm active:scale-[0.99] whitespace-nowrap cursor-pointer">
                ...
                <div class="rounded-xl border border-slate-200/60 overflow-hidden">
                    <div class="bg-slate-50 px-4.5 py-3 border-b border-slate-200 flex justify-between text-xs font-bold text-slate-400 uppercase tracking-wider">
                        ...
                        @forelse($items as $item)
                            <div class="flex items-center justify-between px-4.5 py-3.5 hover:bg-slate-50/50 transition">
                                ...
                                <form action="{{ route('admin.system-masters.destroy', $item) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-slate-400 hover:text-red-600 transition p-1.5 focus:outline-none cursor-pointer" title="Delete parameter">
