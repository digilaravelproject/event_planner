...
class PlanPdfService
{
    ...
    private function buildPdf(UserEventPlan $plan, array $blocks): string
    {
        ...
        foreach ($blocks as [$type, $text]) {
            $fontSize = match ($type) {
                'title' => 21, 'heading' => 12, 'subheading' => 11, 'total' => 14, 'meta' => 8, default => 9
            };
            $leading = $fontSize + 4;
            $maxChars = match ($type) {
                'title' => 42, 'heading' => 55, 'subheading' => 66, default => 82
            };
            ...
            $color = match ($type) {
                'heading' => '1 1 1', 'total', 'subheading' => '0.52 0.02 0.14', 'meta' => '0.38 0.42 0.48', default => '0.10 0.13 0.20'
            };
    ...
    {
        return "0.75 0.75 0.75 rg 150 35 405 0.5 re f\nBT /F1 7 Tf 0.4 0.4 0.4 rg 155 22 Td (Indicative planning and costing summary. Page {$page}) Tj ET\n";
    }
