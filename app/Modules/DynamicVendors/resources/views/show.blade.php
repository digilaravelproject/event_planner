...
<div class="py-8 space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            ...
            <div class="mt-2 flex flex-wrap items-center gap-3"><h1 class="text-2xl font-extrabold text-slate-900">{{ $vendor->name }}</h1><x-dynamic-vendors::status-badge :status="$vendor->status" /></div>
            <p class="mt-1 text-sm text-slate-500">{{ $vendor->category }} · Updated {{ $vendor->updated_at->diffForHumans() }}</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <form method="POST" action="{{ route('admin.dynamic-vendors.duplicate', $vendor) }}">@csrf<button class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700">Duplicate</button></form>
            <a href="{{ route('admin.dynamic-vendors.edit', $vendor) }}" class="rounded-xl bg-[#3950a2] px-5 py-2.5 text-sm font-bold text-white">Edit Vendor</a>
        </div>
    </div>
    ...
    <div class="grid gap-6 xl:grid-cols-3">
        <div class="space-y-6 xl:col-span-2">
                ...
                <div class="mt-5 grid gap-3 sm:grid-cols-2">
                    @forelse(data_get($vendor->vendor_json, 'attributes', []) as $attribute)
                        <div class="rounded-xl border border-slate-200 p-4">
                            ...
                            @if(data_get($attribute, 'validation.help_text'))
                                <div class="mt-3 rounded-lg border border-blue-100 bg-blue-50 p-2.5 text-xs leading-5 text-blue-900">
                                    <span class="font-extrabold">Why this field is needed:</span>
                                    {{ data_get($attribute, 'validation.help_text') }}
                                </div>
                            @endif
                            @php
                                $displayRules = collect($attribute['validation'] ?? [])->except(['help_text']);
                            ...
                            @endphp
                            @if($displayRules->isNotEmpty())<p class="mt-2 text-[11px] text-slate-400">Validation & guidance: {{ $displayRuleText }}</p>@endif
                        </div>
                    @empty
            ...
            <details class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
