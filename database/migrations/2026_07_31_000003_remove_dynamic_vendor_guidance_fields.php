...
return new class extends Migration
{
    ...
    {
        // Removed free-text guidance cannot be reconstructed safely.
    }
        ...
        DB::table($table)->orderBy('id')->chunkById(100, function ($rows) use ($table): void {
            foreach ($rows as $row) {
                ...
                foreach ($document['attributes'] ?? [] as $index => $attribute) {
                    unset(
                        $document['attributes'][$index]['validation']['help_text'],
                        $document['attributes'][$index]['validation']['placeholder'],
                    );
                }
